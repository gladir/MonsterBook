{ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÜ
 ³                                                                         Û
 ³                    Malte Genesis/Cortex du Dialogue                     Û
 ³                                                                         Û
...
Const
 ...
 werInputString=$0006;       { Demande une chaŒne de caractŠres (String)}
 werListBox=$0007;           { Demande d'afficher une boŒte ®ListBox¯ }
 ...
 werClickBox=$000F;          { Demande une boŒte … cliquer }
 ...
 {{$IFNDEF GraphicOS}
  HoleMode:Boolean=False;  { Mode trou‚? Utilise le tableau pour les trous dans
                             l'‚ventualit‚ de rafraŒchissement de l'‚cran
                 ou des sous-fenˆtres... }
 {{$ENDIF}
 PrtScr:Boolean=False;     { Impression de l'‚cran }
 ViewAppTitle:Boolean=True;{ Barre d'application visible?}

  { Ajustement des boŒtes de dialogues }
 ...
 SpcMnu:Byte=2;            { D‚finition le nombre d'espace (en caractŠres)
                             entre chaque Menu d‚roulant. }
 StyleBarMnu:Byte=sbmBadSeal;{ Style de barre de menu }
 StyleBackgroundMenu:Byte=sbmClassic; { Style d'arriŠre plan des menus }
 ...
 WallPaperCfg:WallPaperMode=[wpJuxtap];{ ModŠle d'affichage du papier peint }
...
Const
 ...
 sttMacintosh=1;          { Barre de titre style Macintosh (ğ)}
 ...
 sndOpenWin=4;            { Son d'ouvrir une fenˆtre }
 sndCloseWin=5;           { Son de fermeture de fenˆtre }
...
Type
 ...
 MultiModel=Record
  Name:String[31];        { Identification du modŠle (nom,...)}
  Ext:String[31];         { Extension associ‚e s‚par‚ de point-virgule }
  Code:Word;              { Code d'identification ou num‚ro de s‚rie }
 End;

 FileListBox=Record
  W:Window;               { BoŒte de dialogue … utiliser pour l'affichage }
  RX1,RY1:Byte;           { Position physique de l'affichage dans la fenˆtre }
  Length,Height:Byte;     { Largeur et hauteur en texte de la fenˆtre }
  ...
  Box:Byte;               { Indique dans quel boŒte se trouve actuellement
  ...
  ShowGlyphs:Boolean;     { D‚termine si des ic“nes apparaisse … c“t‚ des
                            nom de fichier de la boŒte de liste de fichiers.}
  ...
  Model:Word;             { D‚finit le modŠle de fichier … rechercher }
  CurrModelPos:Word;      { Position du modŠle dans la liste }
  PMM:^MultiModel;        { Pointeur sur un modŠle multiple }
  StartPath:String;       { R‚pertoire d'origine }
  Path:String;            { Changement de recherche d'origine }
  M:ArrayList;            { ModŠle de recherche support‚ par la boŒte de
                            s‚lection de fichier }
  YD:Byte;                { Additionner … la hauteur de la fenˆtre si des
                            modŠles sont disponibles.}
  ...
  NotFirstTime:Boolean;   { Usage interne seulement, pour dire que ce n'est
                            pas la premiŠre fois qu'il est lanc‚e }
 ...
 PullMnuItem=Record
  ...
  HighChar:Chr;           { CaractŠre surimprim‚ de l'item }
 ...
 PullMnu=Record
  ...
  IconRoutine:Procedure(X,Y:Byte;Level:Word); { Routine devant ˆtre appel‚e pour le tra‡age des Ic“nes }
  OnContext:Function(Code:Word):Boolean;      { Cette routine est appel‚e si le deuxiŠme bouton est enfonc‚e }
 End;
 ...
 ListTitle=Record
  ...
  Title:String;            { Les titres s‚par‚s par le caractŠre ®|¯ }
 End;

 ResourceWindow=Record
  W:Window;                       { Fenˆtre associ‚e }
  IsWindowFixed:Boolean;          { Fenˆtre d‚j… fix‚? }
  ...
  NotFirstTime:Boolean;           { Ce n'est pas la premiŠre fois qu'elle est ex‚cut‚e? }
 ...
 {Enregistrement pour la gestion avanc‚ des fenˆtres}
 ...
 LineImage=Record
  ...
  CharacterTransparent:Boolean; { CaractŠre avec fond transparent? }
 End;

 LstMnu=Record
  List:ArrayList;         { Liste des ‚l‚ments de la liste }
  P:Word;                 { Position absolue dans la liste }
  W:Window;               { BoŒte de dialogue utilis‚ }
  Y:Byte;                 { Position vertical dans la boŒte de dialogue }
  SearchString:String[15];{ ChaŒne de recherche }
  (* Variable d'‚v‚nement externe *)
   { Retourner une valeur de retour que lorsqu'il a cliqu‚ par la deuxiŠme
     fois sur l'‚l‚ment d‚sirer!}
  EnterOnDoubleClick:Boolean;
   { Nombre d'espace r‚serv‚ pour le tra‡age d'Ic“ne }
  Space4Icon:Byte;
   { Routine devant ˆtre appel‚e pour le tra‡age des Ic“nes }
  IconRoutine:Procedure(X,Y:Byte;P:Word;Var Context);
   { AprŠs chaque rafraŒchissement }
  OnRefresh:Procedure(Var L;Var Context);
   { Routine appeler aprŠs un d‚placement }
  ...
  OnClickButton:Procedure(Var Context;P:Word);
   { Routine appeler lorsque le deuxiŠme bouton de la souris est enfonc‚ }
 ...
 End;
  ...
  { Objet de taille relative de la fenˆtre }
 ...
 ColorGridRec=Record
  X,Y:Byte;                  { Coordonn‚e o— se trouve le boŒte }
 ...
 ElementWins=Record
  ...
  Data:Record Case Word of
   ...
   werTextXY:(
    txX,txY:Byte;             { Position o— il doit ˆtre afficher }
   ...
   werClickBox:(
    cbX,cbY:Byte;             { Coordonn‚e o— se trouve le boŒte }
   ...
   werRadioButton:(
    rbDial:SwitchOption;      { BoŒte de dialogue associ‚e }
 ...
 BarProgress=Record
  W:^Window;  { Fenˆtre de dialogue a utilis‚ }
  X,Y:Byte;   { Position dans la fenˆtre }
 End;

  {BoŒte de dialogue de recherche de fichiers}
 ...
 MatrixKrs=Record
  ...
  Desktop:Record
   ...
   DialStatus:Byte; { Couleur des informations d'‚tat sur une boŒte de dialogue }
  End;
  Dialog:Record
   Window:MtxColors;
   Env:Record
...
Var
 ...
 FontTitle:XInf;          { Fond associ‚e … la barre de titre d'une fenˆtre
                            de dialogue quelconque }
 FontInActifTitle:XInf;   { Fond associ‚e … la barre de titre d'une fenˆtre
                            inactif }
 ...
 StrOk:String[8]Absolute LenOk; {Duplication d'une chaŒne de bouton
...
Const
 ...
 FirstTimeCB:Boolean=True; { Est-ce la premiŠre fois que le presse-papier est utilis‚? }
...
Var
 ...
 CurrResWins:^ResourceWindow;   { Ressource de fenˆtre courante }
